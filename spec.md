...
- 2026-06-07 — §3.2.1 Reload/Discard banner on dirty-buffer external changes shipped. The `data-testid="fs-change-reload-needed"` placeholder in the Editor is now a banner with two buttons: Reload and Discard. Both call `queryClient.invalidateQueries` on the note key, which triggers a re-read from disk and replaces the editor content. Both buttons perform the same action in MVP (discard local edits + reload from disk). 2 new tests cover the Reload click and Discard click flows. (`src/components/Editor.tsx`)
- 2026-06-07 — §3.3 real-time link refactor on file rename/move shipped. The `rename_note` IPC command now updates the SQLite index atomically alongside the filesystem rename. `src/index/ingest_incremental.rs::apply_rename` runs a single transaction: `UPDATE documents SET file_path = ?new WHERE file_path = ?old` (preserving the document `id` so outgoing connections survive via FK) and `UPDATE connections SET target_path = ?new WHERE target_path = ?old` (rewriting incoming links). The index update is invoked from `fs/note.rs::rename_note_in` after the `std::fs::rename` succeeds, using the existing `index_db_path` on `AppState`. Errors are logged but do not fail the user-visible rename — the next `rebuild_index` heals the index. 4 unit tests + 2 IPC integration tests cover the happy path, incoming-link refactor, outgoing-link preservation, and the no-op case when the old path doesn't exist in the index. In-app renames only; external-rename detection (OS file manager) is deferred to 3.3.1.
