...
│   │   ├── GraphView.tsx                # ForceGraph2D canvas: debounced resize, simulation reheat, fit-view button, hover glow, 15s refetch
│   │   ├── ToastHost.tsx                # global error/success/info toasts
│   │   └── VaultSwitcher.tsx            # header pill: current vault + recents + close
│   ├── extensions/
│   │   ├── inlineRender.ts              # CodeMirror ViewPlugin: translates RenderedNote spans into Decorations, splits markers from content for all formatting (** * ~~ ` # [text] [[link]]), hides markers via cm-md-marker class + cursor tracking (2.6)
│   │   └── wikilinkHighlight.ts     # ViewPlugin (custom, no MatchDecorator): splits wikilink into bracket + content + bracket decorations, cm-wikilink-bracket + cursor tracking (2.3)
│   ├── env.d.ts
...
| 2026-06-07 | **GraphView overhaul: smooth auto-resize + animation reheat + polish.** Removed `transition-[width] duration-200` from the right-pane aside (`App.tsx`) which was the root cause of canvas jank during resize drag. Added debounced `ResizeObserver` in `GraphView.tsx` so ForceGraph2D only re-renders at the final size. Simulation reheats via `d3ReheatSimulation()` when dimensions change. Auto-centers on the active note when `activePath` changes. Added fit-view button overlay, node hover glow. Memoized all canvas callbacks. Reverted physics to original proven values. Fixed ResizeObserver lifecycle (container div always rendered so observer starts on mount). (`src/components/GraphView.tsx`, `src/App.tsx`, `src/__tests__/GraphView.test.tsx`, `src/__tests__/setup.tsx`) | The CSS width transition on the right pane fired `ResizeObserver` at each intermediate pixel during drag, forcing ForceGraph2D to redraw its canvas continuously. Removing the transition stops the jank. Reheating the simulation on dimension change ensures nodes redistribute after layout settles. Auto-center on note switch replaces broken auto-zoom-to-fit that caused the graph to disappear. |
| 2026-06-08 | **Wikilink bracket hiding: `[[` and `]]` hidden by default, shown only when cursor is inside the wikilink.** Moved to wikilinkHighlight.ts (custom ViewPlugin replaces MatchDecorator). Later generalized to ALL markdown formatting markers: `**`, `*`, `~~`, `` ` ``, `#`, `[text](url)`, `[[link]]` — all hidden when cursor is outside the formatted range. inlineRender.ts now splits every RenderedSpan into marker + content + marker sub-decorations. A unified cursor class `cm-formatting-cursor-inside` is toggled by both plugins. Memory constraint added to AGENTS.md (single vitest process at a time). (`src/extensions/inlineRender.ts`, `src/extensions/wikilinkHighlight.ts`, `src/styles.css`, `AGENTS.md`) | Matches Obsidian Live Preview behavior — markers invisible unless cursor is on the formatted range. |
| 2026-06-10 | **Design System Foundation:** CSS custom properties (`:root` tokens in `src/styles.css`), Fira Code + Fira Sans fonts via Google Fonts in `index.html`, Tailwind config extended with `fontFamily` + brand blue palette (#2563EB, 50-950) + surface color tokens. App shell layout: h-12 header, brand-blue active tab underlines, wider DragHandle (w-2) with brand hover, transition-colors on all interactive elements, font-mono on OBSIDIANA title. EmptyState: branded h1 with 'O' in brand-blue, updated copy, brand-styled "Open vault" button. FileTree: SVG chevron icons replacing ▾/▸, SVG file icon for notes, left-border brand accent on active file, hover transitions, monospace header. (`src/styles.css`, `index.html`, `tailwind.config.ts`, `src/App.tsx`, `src/components/EmptyState.tsx`, `src/components/FileTree.tsx`) | Visual redesign pass — brand identity, consistent color/typography tokens, precursor to full component polish pass. |
| 2026-06-10 | **Right Pane & Editor polish.** Applied design system tokens to remaining UI: Editor header (mono breadcrumb, SVG close icon, brand/success/warning/error status badge), VaultSwitcher (SVG chevron, rounded-md, transitions), IndexStatusChip (10%-opacity colored backgrounds), right pane panels (p-4, mono uppercase headers), BacklinkItem (zinc-200 text), TagChip (zinc/surface tokens). (`src/components/Editor.tsx`, `src/components/VaultSwitcher.tsx`, `src/components/IndexStatusChip.tsx`, `src/components/TagsPanel.tsx`, `src/components/BacklinksPanel.tsx`, `src/components/BacklinkItem.tsx`, `src/components/TagChip.tsx`) | Consistency pass — all right-pane and header components now use brand color/surface tokens. |
