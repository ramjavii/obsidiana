...
│   ├── extensions/
│   │   ├── inlineRender.ts              # CodeMirror ViewPlugin: translates RenderedNote spans into Decorations, splits wikilinks into hidden-bracket sub-decorations, tracks cursor to show/hide brackets (2.6)
...
| 2026-06-08 | **Wikilink bracket hiding: `[[` and `]]` hidden by default, shown only when cursor is inside the wikilink.** Modified `inlineRender.ts` to split resolved wikilink spans into three sub-decorations (`[[`, content, `]]`). Brackets get `cm-md-wikilink-bracket` (`opacity: 0`). A cursor-tracking plugin toggles `cm-wikilink-cursor-inside` on the editor DOM when cursor head is inside any wikilink range, making brackets visible (`opacity: 1`). Memory constraint added to AGENTS.md (single vitest process at a time). (`src/extensions/inlineRender.ts`, `src/styles.css`, `src/__tests__/inlineRender.test.ts`, `AGENTS.md`) | Brackets now match Obsidian behavior: invisible unless cursor is on the link. Click handling unaffected (outer `cm-wikilink` span handles clicks via `closest()`). |
